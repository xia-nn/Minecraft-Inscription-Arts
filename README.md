...
| 物品 | 配方 |
|---|---|
| 萃取台 `extractor_block` | 圆石 3×3 中空，中心铁锭（I） |
| 符文祭坛核心 `rune_altar_core` | 圆石四角(O)、铁锭十字(I)、中心金锭(N) |
...
- **基座**：核心**正下方一层**为 3×3 的**圆石（Cobblestone）**；
- **立柱**：核心**同一层**的四个对角位置，**向上 3 格**放 **原木（Logs，任意种类）**。

```
俯视（核心所在层）：          侧视（核心在中间）：
. O .                        Y+2 : O . O
O C O  (C=核心, O=原木立柱)   Y+1 : O . O
. O .                        Y   : O C O
                                  Y-1 : B B B  (圆石基座 3×3，核心正下方一层)
O=原木(任意种类)  C=符文祭坛核心  B=圆石
```
...
- **祭坛提示无效？** 检查：核心正下方一层是否为 3×3 圆石，且核心同层四角向上 3 格是否为原木。
